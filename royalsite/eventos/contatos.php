...
<body>
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">			
		...
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header">Atender Contato</h1>
			</div>
		</div>	
            <?php
                    ...
                    function get_post_action($name)
                        ...
                        switch (get_post_action('add', 'carregar','submit')) {
                            case 'add':
                                        ...
                                        if(($deletar != '')){
                                            $sql2 = "DELETE FROM contato WHERE id_contato = '$deletar'";
                                            mysql_query($sql2);
                                           // echo "Mostrar value atual: ".$deletar;
                                            echo "Contato excluido com sucesso, atendimento feito com sucesso ! Estamos lhe direcionando em 3 segundos.";
                                            $deletar = '';
                            ...
                            case 'carregar':
                                    ...
                                    $sql3 = "SELECT * FROM contato WHERE id_contato =$id_atual";
                                    $resul = mysql_query($sql3, $conexao);
                                    while($linha = @mysql_fetch_row($resul)) { 
                                    	
                                        $nome_contato_selecionado = $linha[1];
                                        $email_selecionado = $linha[2];
                                        $telefone_selecionado = $linha[3];
										$mensagem_selecionado = $linha[4];
					...
					<script> 
						...
						function olhacampo3(){
								...
								document.getElementById("imagem").style.backgroundColor = "#FF4040";}
                                        ...
                                        <form style="background-color:#F8F8FF; padding-top:10px;padding-right:5px;" class="navbar-form navbar-left" method="post" action="">
                                             <label>Selecione o Contato a ser atendido:</label>
                                     <?php 
                                            ...
                                            echo  "<option value=\"\"></option>";
                                                    ...
                                                    $resu = mysql_query($sql, $conexao);
                                                        while($linhac = mysql_fetch_row($resu)) { 
                                                            $id_contato = $linhac[0];
                                                            $nome_contato = $linhac[1];
                                                            $email_contato = $linhac[2];
															$telefone = $linhac[3];	
                                                            $conteudo = $linhac[4];
                                                            echo  "<option value=".$id_contato.">".$nome_contato."</option>";
                                                                  }
                                                    echo  "</select>";
                                      ...
                                      ?><br>
                                <input name="add" type="submit" style="width: 20%" class="btn btn-success btn-block"  value="Contato Atendido">
					  ...
					  <div class="form-group">
					   <label>Email Contato:</label>
                                           <input type="text"id="data" name="email" onBlur="olhacampo2()" class="form-control" value="<?php echo @$email_selecionado;?>">
					  </div>
					  <br><hr>
					   <div class="form-group">
					   <label>Telefone Contato:</label>
                         <input type="text" id="imagem" name="telefone" onBlur="olhacampo3()" class="form-control" value="<?php echo @$telefone_selecionado;?>">
					  </div>
					  <br><hr>

					  <div class="form-group">
                                              <textarea id="textarea" name="area"><?php echo $mensagem_selecionado; ?></textarea>
					  </div><br><hr>
